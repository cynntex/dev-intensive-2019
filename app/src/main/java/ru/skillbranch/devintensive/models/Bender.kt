...
class Bender (var status:Status = Status.NORMAL, var question: Question = Question.NAME) {
    ...
    fun askQestion():String = when (question){
    ...
    enum class Question(val question:String, val answers:List<String>){
        ...
        PROFESSION("Назови мою профессию?", listOf("сгибальщик", "bender")) {
            ...
            override fun validateAnswer(answer: String): Boolean = answer.trim().firstOrNull()?.isLowerCase() ?: false
        },
        MATERIAL("Из чего я сделан?", listOf("металл", "дерево", "metal", "iron", "wood")) {
            override fun nextQuestion(): Question = BDAY
            override fun validateAnswer(answer: String): Boolean = answer.trim().contains(Regex("\\d")).not()
        },
        ...
        SERIAL("Мой серийный номер?", listOf("2716057")) {
            ...
            override fun validateAnswer(answer: String): Boolean = answer.trim().contains(Regex("^[0-9]{7}$"))
        },
